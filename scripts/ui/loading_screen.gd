...
func start_loading_animation() -> void:
	...
	tween.tween_property(texture_rect, "self_modulate", Color(0,0,0,1), 0.3).from(Color(0,0,0,0))
	await tween.finished


func end_loading_animation() -> void:
	var tween = create_tween()
	tween.tween_property(texture_rect, "self_modulate", Color(0,0,0,0), 0.3).from(Color(0,0,0,1))
	await tween.finished
